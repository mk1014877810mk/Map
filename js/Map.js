$(function () {
  ...
  geolocation.getCurrentPosition(function (position) {
    ...
    var opts = { offset: new BMap.Size(40, 50) }   // 设置比例尺的位置
    map.addControl(new BMap.ScaleControl(opts));   // 添加比例尺控件  
    var ops1 = { offset: new BMap.Size(40, 60) }    // 设置地图放大缩小按钮的位置
      ...
      if (placeBegin == "我的位置") {
        ...
        $.ajax({
          ...
          success: function (data) {
            if (data.status == 0) {
              // console.log(data);
              placeBegin = data.result.addressComponent.city + data.result.addressComponent.district + data.result.addressComponent.street;
              // console.log(p1)
    ...
    function setHeight() {
      ...
      timer = setInterval(function () {
        var top = $("#r-start").height() + $("#r-end").height() + $(".method").height() + $("#r-result>div>h1").height() + $("#r-result>div>.navtrans-view.expand").height() + $(".select").height() + 85;
    ...
    function setBusHeight() {
      ...
      timer = setInterval(function () {
        var top = $("#r-start").height() + $("#r-end").height() + $(".method").height() + $(".select").height() + 10;
        var height = $(window).height() - top;
    ...
    function s_foot() {
      ...
      $.ajax({
        ...
        success: function (data) {
          if (data.status == 0) {
            // console.log(data);
            p1 = data.result.addressComponent.city + data.result.addressComponent.district + data.result.addressComponent.street;
            // p1 = data.result.formatted_address;
            // console.log(p1)
